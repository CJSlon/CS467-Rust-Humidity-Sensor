...
use embassy_rp::{
    bind_interrupts,
    i2c::{self, AbortReason, Async, Config as I2cConfig, I2c, InterruptHandler},
    peripherals::I2C0,
};
...
async fn main(_spawner: Spawner) {
    ...
    match dht20_init(&mut dht20_i2c, DHT20_ADDRESS, DHT20_STATUS).await {
        Ok(()) => {
            ...
            loop {
                let (busy, read_err, write_err, data) =
                    get_humidity_sensor_data(&mut dht20_i2c).await;
                ...
                if write_err {
                    ...
                    loop {
                        ...
                        let filtered_humidity =
                            filter_iir(humidity, prev_filtered_humidity, IIR_ALPHA);
